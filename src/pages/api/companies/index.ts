...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createCompany() {
    ...
    const body = { ...req.body };
    if (body?.digital_marketing?.length > 0) {
      const create_digital_marketing = body.digital_marketing;
      body.digital_marketing = {
        create: create_digital_marketing,
      };
    } else {
      delete body.digital_marketing;
    }
    if (body?.visual?.length > 0) {
      const create_visual = body.visual;
      body.visual = {
